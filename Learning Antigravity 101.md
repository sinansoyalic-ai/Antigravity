...
mindmap
  root((Antigravity))
    ...
    Transition
      From Navigator Cursor
      To Director Antigravity
    Main Features
      Browser Subagent
      Artifact Lifecycle
      Planning Mode
...
```mermaid
graph TD
  Prompt((User Prompt)) --> PlanMode[Planning Mode]
  PlanMode --> TaskArt{Task Artifact}
  TaskArt --> ImpPlan[/Implementation Plan/]
  ImpPlan --> Exec[Execution]
  Exec --> Verif[(Browser Verification)]
  Verif --> MediaArt([Media Artifact])
```
